...
function completar_campos(propietario) { // este método carga los datos de un registro en el formulario de actualización
    console.log(propietario);
    $('#apellido').val(propietario.persona.apellido);
    $('#dni').val(propietario.persona.dni);
    $('#sexo').val(propietario.persona.sexo);
    $('#fecha_nac').val("");
    $('.datepicker').bootstrapMaterialDatePicker ('setDate', moment(propietario.persona.fecha_nac));
    ...
    $("#modal-update").on('shown.bs.modal', function () {
        ...
        instanciar_croppie_update(url_imagenes_personas + propietario.persona.foto_perfil);
    }); 
}
