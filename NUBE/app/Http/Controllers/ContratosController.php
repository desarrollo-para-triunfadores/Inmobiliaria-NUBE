...
class ContratosController extends Controller {
    ...
    public function store(Request $request) {
        ...
        if (is_null($request->inquilino_id)) { // si no se recibe un inquilino se crea uno
        ...
        /* ---------------------INICIO ENVIO EMAIL A INTEREZADOS------------------- */


        $oportunidades = Oportunidad::where('inmueble_id', $contrato->inmueble_id)->get();

        foreach ($oportunidades as $oportunidad) {
            Mail::send('emails.email_inmuebleYaNoDisponible', function ($msj) {
                $msj->subject('Cloudprop | Mensaje de información');
                $msj->to($oportunidad->email);
            });
            return response()->json(json_encode("Se envio el email", true));
        }

        /* ---------------------FIN ENVIO EMAIL A INTEREZADOS------------------- */
        /* ---------------------INICIO OPORTUNIDADES------------------ */

        foreach ($oportunidades as $oportunidad) {
            $oportunidad->estado_id = 5; //este valor hay que cambiarlo por otro después. Según la lógica debería existir en la base de datos un valor que simbolice a las oportunidades que fueron desafectadas.
            $oportunidad->save();
        }
